<?php	
	...
	if (get_option('thecorporation_use_pages') == 'false') query_posts("showposts=$featured_num&cat=".get_catId($featured_cat));
		...
		if (get_option('thecorporation_feat_pages') <> '') $featured_num = $pages_number - count(get_option('thecorporation_feat_pages'));
		...
		query_posts(array
						...
						'orderby' => 'menu_order',
						'order' => 'ASC',
						'post__not_in' => get_option('thecorporation_feat_pages'),
