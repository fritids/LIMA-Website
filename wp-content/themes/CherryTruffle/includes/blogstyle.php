<?php if (is_home()) {
		if (get_option('cherrytruffle_duplicate') == 'false') {
			$args=array(
			   ...
			   'post__not_in' => $ids,
			   'paged'=>$paged,
			   'category__not_in' => get_option('cherrytruffle_exlcats_recent'),
			);
		} else {
			$args=array(
			   'showposts'=>get_option('cherrytruffle_homepage_posts'),
...
<div id="container2"> <img src="<?php bloginfo('template_directory'); ?>/images/content-top-home-2.gif" alt="logo" style="float: left;" />
    <div id="left-div">
<?php if (have_posts()) : while (have_posts()) : the_post(); ?>
        <div class="post-wrapper" style="margin-bottom: 40px;">
			...
			<?php if (get_option('cherrytruffle_thumbnails_index') == 'on') { ?>
				<?php $width = get_option('cherrytruffle_thumbnail_width_index');
					  $height = get_option('cherrytruffle_thumbnail_height_index');
					  $classtext = 'thumbnail';
    ...
    <?php get_sidebar(); ?>
    <img src="<?php bloginfo('template_directory'); ?>/images/content-bottom-2.gif" alt="logo" style="float: left;" /> </div>